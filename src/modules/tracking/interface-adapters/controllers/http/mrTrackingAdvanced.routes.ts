...
export const mrTrackingAdvancedRoutes: FastifyPluginAsync<MrTrackingAdvancedRoutesOptions> = async (
  ...
  fastify.post<{ Body: { mrId?: string; projectPath?: string } }>(
    ...
    async (request, reply) => {
      ...
      const manualFollowupProcessor = async (
        job: ReviewJob,
        signal: AbortSignal,
      ): Promise<void> => {
        ...
        try {
          ...
          try {
            diffMetadata = diffMetadataFetchGateway.fetchDiffMetadata(
              job.projectPath,
              job.mrNumber,
            );
          } catch (error) {
        ...
        if (result.success) {
          ...
          try {
            ...
            logger.warn(
              { mrNumber: job.mrNumber },
              'Failed to fetch diff stats for manual followup',
            );
          }
      ...
      if (gateClaudeInvocation) {
        ...
        if (gateResult.status === 'pending') {
          logInfo('Manual followup parked for human confirmation', {
            mrId,
            mrNumber,
  ...
  fastify.post<{ Body: { mrId?: string; projectPath?: string; enabled?: boolean } }>(
    '/api/mr-tracking/auto-followup',
    async (request, reply) => {
      const { mrId, projectPath, enabled } = request.body;
  ...
  fastify.post<{ Body: { projectPath?: string } }>(
    '/api/mr-tracking/followup-importants',
    async (request, reply) => {
      ...
      logInfo('Followup importants batch triggered', {
        triggered,
  ...
  fastify.post<{ Body: { projectPath?: string; mrId?: string } }>(
    '/api/mr-tracking/sync',
    async (request, reply) => {
      const { projectPath, mrId } = request.body;
