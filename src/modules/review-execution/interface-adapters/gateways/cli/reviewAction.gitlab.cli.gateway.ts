  ...
  implements ReviewActionGateway
{
  protected buildCommand(action: ReviewAction, context: ExecutionContext): CommandInfo | null {
    ...
    switch (action.type) {
      ...
      case 'POST_COMMENT': {
        const enrichedBody =
          context.baseUrl && context.diffMetadata
            ? enrichCommentWithLinks(
                action.body,
                context.baseUrl,
                context.projectPath,
                context.diffMetadata.headSha,
              )
            : action.body;
        return {
          command: 'glab',
      ...
      case 'POST_INLINE_COMMENT': {
        ...
        return {
          ...
          args: [
            ...
            `body=${action.body}`,
            '--field',
            `position[base_sha]=${context.diffMetadata.baseSha}`,
            '--field',
            `position[head_sha]=${context.diffMetadata.headSha}`,
            '--field',
            `position[start_sha]=${context.diffMetadata.startSha}`,
            '--field',
            'position[position_type]=text',
            '--field',
            `position[new_path]=${action.filePath}`,
            '--field',
            `position[old_path]=${action.filePath}`,
            '--field',
