...
export type ProgressChangeCallback = (jobId: string, progress: ReviewProgress) => void;
...
export function createMcpDependencies(input: McpDependenciesInput): McpDependencies {
  ...
  progressGateway.setOnProgressChange((jobId, progress) => {
    ...
    syncProgressToReviewContext(jobId, progress, jobContextGateway, reviewContextGateway);
  });
