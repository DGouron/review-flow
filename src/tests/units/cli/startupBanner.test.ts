...
describe('formatStartupBanner', () => {
  ...
  it('should return dashboard URL', () => {
    ...
    expect(result.lines.some((line) => line.includes('http://localhost:3000/dashboard/'))).toBe(
      true,
    );
  });
  ...
  it('should include GitLab webhook URL when gitlab is enabled', () => {
    const result = formatStartupBanner({
      port: 4000,
      enabledPlatforms: ['gitlab'],
      daemonPid: null,
    });

    expect(
      result.lines.some((line) => line.includes('http://localhost:4000/webhooks/gitlab')),
    ).toBe(true);
  });
  ...
  it('should include GitHub webhook URL when github is enabled', () => {
    ...
    expect(
      result.lines.some((line) => line.includes('http://localhost:4000/webhooks/github')),
    ).toBe(true);
  });
