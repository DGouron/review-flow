...
describe('MarkReviewDoneUseCase', () => {
  ...
  it('applies the done label to the requested merge request', async () => {
    await useCase.execute(TARGET);

    expect(reviewLabelGateway.ensured).toEqual([
      { projectPath: TARGET.projectPath, label: REVIEW_DONE_LABEL },
    ]);
    expect(reviewLabelGateway.added).toEqual([{ ...TARGET, label: REVIEW_DONE_LABEL }]);
  });
