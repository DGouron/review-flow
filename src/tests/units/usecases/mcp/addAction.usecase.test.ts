...
import { addAction } from '@/modules/review-execution/usecases/mcp/addAction.usecase.js';
...
describe('addAction usecase', () => {
  ...
  it('should return error when THREAD_RESOLVE missing threadId', () => {
    const result = addAction(jobId, { type: 'THREAD_RESOLVE' } as never, {
      jobContextGateway,
  ...
  it('should return error when THREAD_REPLY missing threadId', () => {
    const result = addAction(jobId, { type: 'THREAD_REPLY', message: 'test' } as never, {
      jobContextGateway,
      reviewContextGateway,
    });
  ...
  it('should return error when POST_COMMENT missing body', () => {
    const result = addAction(jobId, { type: 'POST_COMMENT' } as never, {
      jobContextGateway,
      reviewContextGateway,
    });
