...
describe('extractBaseUrl', () => {
  ...
  it('should extract protocol://host from HTTP URL', () => {
    expect(extractBaseUrl('http://gitlab.example.com/group/project.git')).toBe(
  ...
  it('should preserve port in HTTPS URL', () => {
    expect(extractBaseUrl('https://gitlab.example.com:8443/group/project.git')).toBe(
      'https://gitlab.example.com:8443',
    );
  });

  it('should extract host from SSH URL with nested groups', () => {
    expect(extractBaseUrl('git@gitlab.example.com:group/subgroup/project.git')).toBe(
      'https://gitlab.example.com',
    );
  });
